...
export default class GamePlayer {
  ...
  explosionSfx!: PlayerExplosionSfx;
  ...
  onCollisionBegan(edges: CollisionEdges) {
  ...
  getJumpForce(inputs: Record<string, boolean>) {
    const { opts } = this;
  ...
  getWalkingForce(inputs: Record<string, boolean>) {
    const { opts } = this;
    let force = 0;
    if (inputs.Left) {
  ...
  processInputs(inputs: Record<string, boolean>) {
