...
export default class InputManager {
  ...
  keyboardEventDelegate!: any;
  gamepadEventDelegate!: any;
  gamepadButtonEventDelegate!: any;
  gamepadAxeEventDelegate!: any;
  ...
  toggleBindings(action: any) {
    // KeyboardInstance.events.on('keyup', this.keyboardEventDelegate);
    (KeyboardInstance.events as any)[action]('keydown', this.keyboardEventDelegate);
    (GamepadInstance.events as any)[action]('gamepadbutton', this.gamepadButtonEventDelegate);
    (GamepadInstance.events as any)[action]('gamepadaxes', this.gamepadAxeEventDelegate);
    (GamepadInstance.events as any)[action]('gamepadconnected', this.gamepadEventDelegate);
    (GamepadInstance.events as any)[action]('gamepaddisconnected', this.gamepadEventDelegate);
  }
  ...
  handleGamepadButtonEvent(e: any) {
  ...
  handleGamepadAxeEvent(e: any) {
    const { gamepadBindings } = this.opts;
    const { name } = e.detail;
    const binding = gamepadBindings[name];
    if ((GAMEPAD_AXES as any)[name] && typeof binding === 'function') {
      binding(e);
    }
  }

  handleGamepadEvent(e: any) {
    const { gamepadBindings } = this.opts;
    const { type } = e;
    const binding = gamepadBindings[type];
    if ((GAMEPAD_EVENTS as any)[type] && typeof binding === 'function') {
      binding(e);
    }
  }
